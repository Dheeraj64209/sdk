...
class SipParser {
  static SipMessage parse(String raw) {
    ...
    final contentLengthHeader = headers['Content-Length'] ?? headers['l'];
    final contentLength = int.tryParse((contentLengthHeader ?? '').trim());
    if (contentLength != null && contentLength >= 0 && body.length >= contentLength) {
...
class SipBuilder {
  static String register({
    ...
    // Add transport=ws to Contact so Vobiz knows to route via WebSocket
    final contact = '<sip:$username@vobiz-client;transport=ws>';
  ...
  static String ok200({
    ...
    return <String>[
      ...
      'Contact: <sip:$fromUser@vobiz-client;transport=ws>',
      'Allow: INVITE, ACK, CANCEL, BYE, OPTIONS',
      'Supported: timer',
      'User-Agent: vobiz_mobile_flutter',
      'Content-Type: application/sdp',
...
class SIPService {
  ...
  SipMessage? _lastAnsweredInvite;
  String? _lastAnsweredSdp;
  ...
  Future<void> connect() async {
    ...
    try {
      print('🔌 [SIP] Connecting to WebSocket: $wsUrl');
  ...
  void answer(String sdp) {
    ...
    _lastAnsweredInvite = _pendingInvite;
    _lastAnsweredSdp = sdp;
    _send(
      SipBuilder.ok200(
  ...
  void _handleMessage(String raw) {
    ...
    // Debug: Log ALL incoming SIP messages
    print('🔵 [SIP RECV] ========================================');
    print(raw.substring(0, raw.length > 500 ? 500 : raw.length));
    print('🔵 [SIP RECV] ========================================');
  ...
  void _handleRequest(SipMessage msg) {
    print('📨 [DEBUG] Received SIP Request: ${msg.method}');
    switch (msg.method) {
      case 'INVITE':
        print('📞 [DEBUG] INVITE request detected! Calling _handleIncomingInvite...');
  ...
  void _handleIncomingInvite(SipMessage msg) {
    print('🔔 [DEBUG] _handleIncomingInvite called!');
    print('🔔 [DEBUG] _activeCallId: $_activeCallId');
    print('🔔 [DEBUG] _pendingInvite: $_pendingInvite');
    print('🔔 [DEBUG] INVITE body length: ${msg.body.length}');
    print('🔔 [DEBUG] INVITE body preview: ${msg.body.substring(0, msg.body.length > 300 ? 300 : msg.body.length)}');
    
    final incomingCallId = msg.header('Call-ID');

    // If we are already handling a call, handle retransmits for the same dialog.
    if (_activeCallId != null || _pendingInvite != null) {
      final isSameDialog = incomingCallId != null && incomingCallId == _activeCallId;
      if (isSameDialog) {
        if (_pendingInvite != null) {
          // INVITE retransmit before answer: repeat 180 Ringing.
          print('↩️ [DEBUG] INVITE retransmit for same call - re-sending 180 Ringing');
          _send(SipBuilder.ringing(invite: msg, localTag: _localTag));
          return;
        }
        if (_lastAnsweredInvite != null &&
            _lastAnsweredSdp != null &&
            _lastAnsweredSdp!.isNotEmpty) {
          // INVITE retransmit after answer: repeat 200 OK with SDP.
          print('↩️ [DEBUG] INVITE retransmit for answered call - re-sending 200 OK');
          _send(
            SipBuilder.ok200(
              invite: _lastAnsweredInvite!,
              fromUser: _username!,
              localTag: _localTag,
              sdp: _lastAnsweredSdp!,
            ),
          );
          return;
        }
      ...
      print('❌ [DEBUG] Rejecting as busy!');
      _send(SipBuilder.busyHere(invite: msg, localTag: _localTag));
      return;
    }

    _pendingInvite = msg;
    _activeCallId = msg.header('Call-ID');
    final callerId = _extractIncomingCallerId(msg);
    final parsedTo = _extractUserFromSipHeader(msg.header('To') ?? '');
    _activeToUser = parsedTo == 'Unknown' ? callerId : parsedTo;
    final fromHeader = msg.header('From') ?? '';
    _activeRemoteTag = _extractTag(fromHeader);
    _activeRemoteTarget = _extractUri(msg.header('Contact') ?? fromHeader);
    _activeRemotePartyHeader = fromHeader;
    
    print('✅ [DEBUG] Sending 180 Ringing...');
    _send(SipBuilder.ringing(invite: msg, localTag: _localTag));
    
    print('✅ [DEBUG] Calling onIncomingCall callback with callerId: $callerId');
    print('✅ [DEBUG] Passing SDP body of length: ${msg.body.length}');
  ...
  void _handleRegisterResponse(SipMessage msg) {
    switch (msg.statusCode) {
      case 200:
        print('✅ [SIP] REGISTERED successfully as $_username');
        print('📱 [SIP] Ready to receive inbound calls!');
  ...
  String _extractUserFromSipHeader(String header) =>
      RegExp(r'sip:([^@>]+)').firstMatch(header)?.group(1) ?? 'Unknown';

  String _extractIncomingCallerId(SipMessage msg) {
    final headers = <String>[
      msg.header('P-Asserted-Identity') ?? '',
      msg.header('Remote-Party-ID') ?? '',
      msg.header('P-Preferred-Identity') ?? '',
      msg.header('X-Caller-ID') ?? '',
      msg.header('X-Original-Caller-ID') ?? '',
      msg.header('Diversion') ?? '',
      msg.header('From') ?? '',
      msg.header('Contact') ?? '',
    ];

    for (final header in headers) {
      final displayNameMatch = RegExp(r'"(\+?\d{8,15})"').firstMatch(header);
      if (displayNameMatch != null) {
        return displayNameMatch.group(1)!;
      }

      final sipUser = _extractUserFromSipHeader(header);
      if (sipUser != 'Unknown' && !_looksLikeOurOwnSipUser(sipUser)) {
        return sipUser;
      }

      final phoneMatch = RegExp(r'\+?\d{8,15}').firstMatch(header);
      if (phoneMatch != null) {
        return phoneMatch.group(0)!;
      }
    }

    return _extractUserFromSipHeader(msg.header('From') ?? '');
  }

  bool _looksLikeOurOwnSipUser(String value) {
    if (_username == null) return false;
    return value == _username ||
        value.contains('@registrar.vobiz.ai') ||
        value.contains('registrar.vobiz.ai');
  }
